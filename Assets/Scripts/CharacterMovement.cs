...
public class CharacterMovement : MonoBehaviour
{
    ...
    IEnumerator characterMovement()
    {
        while (on)
        {
            ...
            currentTurn.getGameobject().transform.position = Vector3.Lerp(screenPositions[indexTile], screenPositions[indexTile + 1], interpolationRatio);
            ...
            if (elapsedFrames >= interpolationFramesCount)
            {
                ...
                {
                    currentTurn.getGameobject().transform.position = screenPositions[indexTile];
                    on = false;
                }
